...
def print_slim_model(rho, X_names, Y_name, show_omitted_variables = False):
    ...
    if '(Intercept)' in rho_names and (sum(rho_values) != rho_values[0]):
...
def get_slim_summary(slim_IP, slim_info, X, Y):
    ...
    slim_summary = {
        ...
        'objective_value': slim_IP.Objective().Value(),
        # 'optimality_gap': slim_IP.Objective().,
        # 'objval_lowerbound': slim_IP.solution.MIP.get_mip_relative_gap(),
        'simplex_iterations': slim_IP.iterations(),
    ...
    rhos = slim_info['rhos']
    rho = np.array([rhos[j].SolutionValue() for j in range(slim_info['P'])])
